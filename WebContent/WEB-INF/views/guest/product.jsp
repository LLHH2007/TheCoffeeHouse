...
-->
			<div class="well well-small">
				...
				<c:forEach var="item" items="${productByPageId}" varStatus="count">
					...
					<li class="span4">
						<div class="thumbnail">
							<a class="overlay" href="<c:url value="/productDetails/${item.id}"/>"></a> <a
								title="Mua hàng" class="zoomTool" href="<c:url value="/productDetails/${item.id}"/>"><span
			...
			<c:forEach var="item" items="${linkOfPage}" varStatus="count">
				<c:choose>
					<c:when test="${count.count==activePage}">
						<a class="active" href="<c:url value="${item }"/>">${count.count}</a>
					</c:when>
					<c:otherwise><a href="<c:url value="${item }"/>">${count.count}</a></c:otherwise>
				</c:choose>
			</c:forEach>
			<a href="#">&raquo;</a>
		</div>
